First, read the AGENTS.md file in the wiki `~/Documents/git_perso/doc/AGENTS.md`

# Weekly reporting

Weekly reporting on activities is drafted in the Wiki in "work/PPP".
* When asked to "add to ppp" or when the command /ppp is run, summerize the current task in the current week entry.
* Create the weekly entry in the file if it does not exist.
* Reporting should be concise and to the point. Take example on previous entries when in doubt
